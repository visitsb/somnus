class Service {
    ...
    async _updateSpec(spec) {
        ...
        try {
            ...
            // For single images retry with `json` again; `somnus` itself gave this exception when updating first time
            latestTags = await jq.run(`.`, latestTags, {input: 'json', output: 'json', slurp: true});
        }
