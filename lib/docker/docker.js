class Docker {
    ...
    async pruneUnusedVolumes() {
        this.logger.verbose(`Pruning all unused volumes...`);
        await this.docker.pruneVolumes();
        this.logger.info(`Successfully pruned all unused volumes`);
    }

    async pruneUnusedNetworks() {
        this.logger.verbose(`Pruning all unused networks...`);
        await this.docker.pruneNetworks();
        this.logger.info(`Successfully pruned all unused networks`);
    }
    ...
    async _listServices() {
        ...
        if (0 === servicesList.length) return [/* No services running */];
