...
class App {
    ...
    async init() {
        ...
        if (args.config) {
            ...
            try {
                ...
                this.config = yaml.load(fs.readFileSync(args.config, 'utf8'));
    ...
    async updateServices() {
        ...
        if (this.isDryRun) {
            this.logger.warn(`Dry-run will not prune any containers, images, volumes and networks. Skipping...`);
        } else {
            // Keep the state clean of `stopped` containers, unused images, volumes, networks that may have been left after updating the service
            this.logger.verbose(`Pruning unused containers, images, volumes and networks...`);
            await this.docker.pruneUnusedContainers();
            await this.docker.pruneUnusedImages();
            await this.docker.pruneUnusedVolumes();
            await this.docker.pruneUnusedNetworks();
        }
